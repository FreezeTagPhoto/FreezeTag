...
def read_message() -> Message:
    msg_byte: bytes = stdin.buffer.read(1)
    if stdin.closed or len(msg_byte) == 0:
    ...
    match msg_type:
        ...
        case MessageType.BIN | MessageType.LOG | MessageType.ERR:
            msg_size_buf: bytes = stdin.buffer.read(8)
            msg_size: int = int.from_bytes(msg_size_buf, byteorder='little', signed=False)
            msg_contents: bytes = stdin.buffer.read(msg_size)
            return Message(msg_type, msg_contents)
        case _:
            msg_size_buf: bytes = stdin.buffer.read(8)
            msg_size: int = int.from_bytes(msg_size_buf, byteorder='little', signed=False)
            msg_bytes: bytes = stdin.buffer.read(msg_size)
...
def write_message(msg: Message):
    ...
    stdout.buffer.write(bytes([msg.mtype.value]))
    match msg.mtype:
        case MessageType.READY | MessageType.SHUTDOWN:
        ...
        case MessageType.BIN | MessageType.LOG | MessageType.ERR:
            if isinstance(msg.contents, bytes):
                ...
                stdout.buffer.write(int.to_bytes(msg_size, 8, byteorder='little', signed=False))
                stdout.buffer.write(msg.contents)
            else:
        ...
        case _:
            ...
            stdout.buffer.write(int.to_bytes(msg_size, 8, byteorder='little', signed=False))
            stdout.buffer.write(msg_contents)
    stdout.buffer.flush()
